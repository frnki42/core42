...
{
	game->tex_empty = mlx_xpm_file_to_image(game->mlx, "textures/empty.xpm", &game->tex_width, &game->tex_height);
	if (!game->tex_empty)
		exit_game(game);
	game->tex_wall = mlx_xpm_file_to_image(game->mlx, "textures/wall.xpm", &game->tex_width, &game->tex_height);
	if (!game->tex_wall)
		exit_game(game);
	...
	if (!game->tex_collectible)
		exit_game(game);
	game->tex_exit = mlx_xpm_file_to_image(game->mlx, "textures/exit_unlocked.xpm", &game->tex_width, &game->tex_height);
	if (!game->tex_exit)
		exit_game(game);
